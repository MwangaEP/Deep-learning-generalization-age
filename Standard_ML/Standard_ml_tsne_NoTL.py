...
# of Anopheles arabiensis mosquitoes reared in different insectaries (Ifakara and Glasgow), No transfer learning. 
...
glasgow_data = pd.read_csv("C:\Mannu\QMBCE\Thesis\glasgow_data.dat", delimiter = '\t')
print(glasgow_data.head())

# Checking class distribution in glasgow data 
print(Counter(glasgow_data["Age"]))

# drops columns of no interest
glasgow_data = glasgow_data.drop(['Species', 'Status', 'Country', 'RearCnd', 'StoTime'], axis=1)
glasgow_data.head(10)
...
glasgow_data['Age_group_new'] = Age_group_new

# Drop age column which contain the chronological age of the mosquito, and 
# keep age classes

glasgow_data= glasgow_data.drop(['Age'], axis = 1)
glasgow_data.head(5)
...
X_valid = np.asarray(glasgow_data.iloc[:,:-1])
y_valid = np.asarray(glasgow_data["Age_group_new"])
