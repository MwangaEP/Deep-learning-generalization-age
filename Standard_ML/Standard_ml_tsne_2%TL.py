...
# of Anopheles arabiensis mosquitoes reared in different insectaries (Ifakara and Glasgow) (2% transfer learning)
...
def plot_confusion_matrix(cm, classes,
    ...
    plt.savefig(("C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\Confusion_Matrix_" + figure_name + "_" + ".png"), dpi = 500, bbox_inches="tight")
...
training_df = pd.read_csv("C:\Mannu\QMBCE\Thesis\Ifakara_data.dat", delimiter = '\t')
print(training_df.head())

# Checking class distribution in the data
print(Counter(training_df["Age"]))

# drops columns of no interest
training_df = training_df.drop(['Species', 'Status', 'Country', 'RearCnd', 'StoTime'], axis=1)
training_df.head(10)
...
# 
# Drawback: It is possible to get different results because it is a probabilistic algorithm
...
tsne_embedded = tsne.fit_transform(X_trans)
...
for round in range(num_rounds):
    ...
    for train_index, test_index in kf.split(tsne_embedded, y):

        # Split data into test and train

        X_train, X_test = tsne_embedded[train_index], tsne_embedded[test_index]
...
classifier.save_model('C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\classifier')
...
rf_per_class_acc_distrib.dropna().to_csv("C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\_rf_per_class_acc_distrib.csv")
rf_per_class_acc_distrib = pd.read_csv("C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\_rf_per_class_acc_distrib.csv", index_col=0)
...
rf_per_class_acc_distrib_describe.to_csv("C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\_rf_per_class_acc_distrib.csv")
...
plt.grid(False)
plt.tight_layout()
plt.savefig("C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\_rf_per_class_acc_distrib.png", dpi = 500, bbox_inches="tight")
...
classifier = XGBClassifier() 
transfer_model = classifier.fit(age_tsne_trans, y_new, xgb_model = 'C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\classifier') # or classifier.get_booster()

with open('C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\Transfer_model.pkl', 'wb') as fid:
...
glasgow_unseen_df = glasgow_unseen_df.drop(['Age'], axis = 1)
glasgow_unseen_df.head(5)
...
X_valid_trans = scaler.transform(X = X_valid)

age_tsne_valid = tsne.fit_transform(X_valid_trans)
...
# with open('C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\classifier.pkl', 'rb') as fid:
...
cr = cr.iloc[1:]
cr.to_csv('C:\Mannu\QMBCE\Thesis\Fold\Standard_ml\_ml_tsne_2\classification_report_tsne_.csv')

#%%

# plot the confusion matrix for the test data (glasgow data)
figure_name = 'test_tsne_2'
