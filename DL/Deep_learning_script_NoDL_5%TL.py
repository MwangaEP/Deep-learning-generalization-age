...
model_size = [{'type':'c', 'filter':8, 'kernel':4, 'stride':1, 'pooling':1}, 
...
savedir = (outdir+"\_transfer_05_k_fold_publish_01")            
build_folder(savedir, True)
savedir = (outdir+"\_transfer_05_k_fold_publish_01\l")            
...
print('Glasgow unseen data for prediction', glasgow_5unseen_df.head())

# Tranform data with standard scaler 
# define matrix of features and vector of labels

X_valid = np.asarray(glasgow_5unseen_df.iloc[:,1:])
y_valid = np.asarray(glasgow_5unseen_df["Age"])
...
model = tf.keras.models.load_model("C:\Mannu\QMBCE\Thesis\Fold\_transfer_05_k_fold_publish_01\lBaseline_CNN_0_3_Model.tf")
...
transfer_lr_model.save('C:\Mannu\QMBCE\Thesis\Fold\_transfer_05_k_fold_publish_01\Transfer_lr_model_2%.tf')
...
cr = cr.iloc[1:]
cr.to_csv('C:\Mannu\QMBCE\Thesis\Fold\_transfer_05_k_fold_publish_01\classification_report_transfer_learning.csv')
